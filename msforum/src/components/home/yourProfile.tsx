...
const YourProfile = () => {
    ...
    useEffect(() => {
        const fetchStats = async () => {
            try {
                const res = await fetch(`/api/userStats`, { next: { revalidate: 120 } });
            ...
            <div className="flex flex-col gap-6">
                <div className="flex flex-row justify-around items-end text-sm text-gray-700 leading-1 gap-3">
                    <div className="flex flex-col mt-6">
                        ...
                        <span className="font-medium text-lg">msworks</span>
                    </div>
                    <div className="flex flex-col">
                        <span className="block text-gray-500">Subscription</span>
                        <span className="font-medium text-lg">Starter User</span>
                    </div>
                </div>
                <div className="flex flex-row gap-2">
                    <div className="rounded-xl border p-4 text-center bg-white shadow-sm w-1/3">
                        ...
                        <div className="text-xl font-bold text-black">{stats?.articlesPublished}</div>
                    </div>
                    <div className="rounded-xl border p-4 text-center bg-white shadow-sm w-1/3">
                        <div className="text-sm font-semibold text-gray-700">Comments</div>
                        <div className="text-xl font-bold text-black">{stats?.commentsWritten}</div>
                    </div>
                    <div className="rounded-xl border p-4 text-center bg-white shadow-sm w-1/3">
                        <div className="text-sm font-semibold text-gray-700">Upvotes</div>
                        <div className="text-xl font-bold text-black">{stats?.totalUpvotesReceived}</div>
                    </div>
                </div>
            </div>
        </>
